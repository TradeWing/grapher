...
export default class CollectionNode {
    ...
    applyFields(filters, options) {
        ...
        if (Meteor.isServer) {
            // Add filters to option.fields to allow proper client-side filtering
            _.each(filters, (value, field) => {
                ...
                if (!_.contains(['$or', '$nor', '$not', '$and', '$meta', '$text'], field)) {
                    ...
                    // TODO: fix bug where filter is a.b.c and options.fields has a.b
                        // don't add filter
                    // TODO: fix bug where filter is a and options.fields has a.b.c
                        // do add filter, remove all more specific options.fields
